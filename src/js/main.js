...
document.addEventListener("DOMContentLoaded", async () => {
          ...
          UIRenderer.renderPredictionDashboard(predictions, predictOutput);
           ...
           try {
             ...
             if(validForBagan.length){
               ...
               if(globalMap.size===0){
                 validForBagan.forEach(p=> (p.prediction.topScorers||[]).slice(0,1).forEach(pl=>{
                   const key=pl.name+"|"+pl.teamCode; if(!globalMap.has(key)) globalMap.set(key,{name:pl.name,teamCode:pl.teamCode,teamName:pl.teamName,flag:pl.flag,pos:pl.pos,totalActual:0,totalXG:pl.expectedGoals});
                 }));
               }
               const globalPreview = [...globalMap.values()].sort((a,b)=>(b.totalActual-b.totalActual)||(b.totalXG-a.totalXG)).slice(0,8).map((pl,i)=>{
                 const badge=i===0?"🥇":i===1?"🥈":i===2?"🥉":"#"+(i+1);
                 const gol = pl.totalActual>0? pl.totalActual : Math.max(1,Math.round(pl.totalXG));
               ...
               try{
                 ...
                 if(baganOut){
                   ...
                   const virtualRows = (ds.matches||[]).slice(0,8).map((m,i)=>{
                     const pred = validForBagan.find(p=>p.row===i+1);
                     const hasInput = (m?.home||"").trim() || (m?.away||"").trim();
                     if(!hasInput && !pred) return null;
                     return {
                       home: pred ? pred.homeName : (m.home||""),
                       away: pred ? pred.awayName : (m.away||""),
                       score: pred ? `${pred.prediction.homeGoals}:${pred.prediction.awayGoals}` : (m.score||"")
                     };
                   }).filter(Boolean);
  ...
  try{
    ...
    function renderDraftLists(){
      ...
      if(yesListEl){
        yesListEl.innerHTML = st.yes.length ? st.yes.map((name,i)=> `<span style="background:#332200;border:1px solid #ff0;color:#ff0;padding:3px 6px;font-size:0.65rem;display:inline-flex;align-items:center;gap:4px;">✅ ${Security.escapeHtml(name)} <button data-idx="${i}" class="btnDraftRmYes" style="background:#332200;border:1px solid #ff0;color:#fff;padding:0 4px;font-size:0.6rem;cursor:pointer;">x</button></span>`).join("") : `<span style="font-size:0.6rem;color:#666;">Belum ada YES — max 3, klik rekomendasi untuk isi otomatis</span>`;
        yesListEl.querySelectorAll(".btnDraftRmYes").forEach(btn=>{
          btn.addEventListener("click", ()=>{
            const idx = parseInt(btn.dataset.idx,10);
            const cur = DraftService.load();
            cur.yes.splice(idx,1);
            DraftService.save(cur);
            renderDraftLists();
          });
        });
    ...
    async function attachDraftAutocomplete(){
      ...
      attach(yesInput, "btnDraftAddYes");
    }
    ...
    document.getElementById("btnDraftAddYes")?.addEventListener("click", ()=>{
      const v = (yesInput?.value || "").trim();
      if(!v) return;
      const cur = DraftService.load();
      const matches = getActiveMatchesForDraft();
      const r = DraftService.addYes(cur.yes, v, cur.bans, cur.bandar, matches);
      if(!r.ok){ if(recommendOut) recommendOut.innerHTML = `<div style="color:#f55;">⛔ ${Security.escapeHtml(r.msg)}</div>`; return; }
      DraftService.save(cur);
      if(yesInput) yesInput.value = "";
      renderDraftLists();
      if(recommendOut) recommendOut.innerHTML = `<div style="color:#0f0;">✓ YES diset: ${Security.escapeHtml(r.resolved)} (melawan bandar ${cur.bandar[0]||"-"})</div>`;
    });
    ...
    document.getElementById("btnDraftRecommend")?.addEventListener("click", ()=>{
      ...
      const res = DraftService.recommend({ matches, bans: cur.bans, bandar: cur.bandar, yesExisting: cur.yes });
      if(!res.ok){
      ...
      const recHtml = res.recommended.map(p=> `
        <div style="background:#002a00;border:1px solid #0f0;padding:6px;display:flex;justify-content:space-between;align-items:center;">
          ...
          <button class="btnDraftPickYes" data-name="${Security.escapeHtml(p.name)}" style="background:#332200;border:1px solid #ff0;color:#ff0;padding:4px 8px;font-size:0.65rem;cursor:pointer;white-space:nowrap;">+ YES</button>
        </div>
      `).join("");
      const bandarInfo = cur.bandar.length ? `Bandar: ${Security.escapeHtml(cur.bandar.join(", "))} → blok negara ${Security.escapeHtml(cur.bandar.join(", "))} ` : "Tanpa bandar";
      ...
      if(recommendOut) recommendOut.innerHTML = `
        ...
        <div style="font-size:0.55rem;color:#555;margin-top:6px;">Klik <strong>+ YES</strong> untuk masukkan ke voting YES (max 3). YES tidak boleh BAN & tidak satu negara bandar — validasi otomatis.</div>
      `;
      recommendOut.querySelectorAll(".btnDraftPickYes").forEach(btn=>{
        btn.addEventListener("click", ()=>{
          const name = btn.dataset.name;
          const cur2 = DraftService.load();
          if(cur2.yes.length >= DraftService.MAX_YES){ recommendOut.innerHTML += `<div style="color:#f55;margin-top:6px;">⛔ YES sudah penuh ${DraftService.MAX_YES} (cuma 1 melawan bandar)</div>`; return; }
          const norm = DraftService.normalizePlayerName(name);
          if(cur2.yes.some(y=> DraftService.normalizePlayerName(y)===norm)) return;
          cur2.yes.push(name);
          DraftService.save(cur2);
          renderDraftLists();
          btn.textContent = "✓ YES";
          btn.disabled = true;
        });
    ...
    // Enter key support
    [banInput, bandarInput, yesInput].forEach(el=>{
      if(!el) return;
      el.addEventListener("keydown", (e)=>{ if(e.key==="Enter"){ e.preventDefault(); const id = el.id==="draftBanInput"?"btnDraftAddBan": el.id==="draftBandarInput"?"btnDraftAddBandar":"btnDraftAddYes"; document.getElementById(id)?.click(); }});
    });
