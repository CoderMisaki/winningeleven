...
function parseSingleLine(line) {
  ...
  const homeCode = normalizeCountry(cleanHome);
  const awayCode = normalizeCountry(cleanAway);
  if (!teamsDB[homeCode] || !teamsDB[awayCode]) return { error: 'home/away harus negara dari 57 daftar project' };

  return {
    home: teamsDB[homeCode].name,
    homeScore: String(homeScore),
    awayScore: String(awayScore),
    score: `${homeScore}:${awayScore}`,
    away: teamsDB[awayCode].name
  };
}
