...
Object.keys(WE10_FULL_ROSTER).forEach(code => {
  ...
  const teamName = teamsDB[code].name;
...
export function lookupPlayerExact(name, teamCode = null) {
  if (!name) return null;
  const q = name.trim().toLowerCase();
  const code = teamCode ? String(teamCode).toUpperCase() : null;
  const candidates = code ? allPlayers.filter((p) => p.teamCode === code) : allPlayers;
