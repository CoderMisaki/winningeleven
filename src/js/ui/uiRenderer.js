...
import { PredictionService, PREDICTOR_CONFIG, OFFICIAL_57_LIST } from "../services/predictor.js";
...
export const UIRenderer = {
  ...
  renderPredictionDashboard(predictions, container) {
    ...
    // GHIDRA VALIDATION BANNER — bukan dummy
    const ghidraBanner = document.createElement("div");
    ghidraBanner.style.cssText = "font-size:0.6rem;color:#0ff;background:#001a33;border:1px solid #0ff;padding:6px 8px;margin-bottom:8px;font-family:var(--font-mono);";
    ghidraBanner.innerHTML = `✅ <strong>IDENTITAS DARI DATA PROJECT</strong> | RNG <code>Xorshift32</code> | Dispatcher <code> Konami Cup</code> | Chances <code>4-20</code> | Shot <code>0.38 + atribut pemain + quality - defense lawan</code> | Difficulty ★5 <code>*1.35</code> | Pool <code>B1-B8 only</code> • <span style="color:#0f0;">Skor = event chance→pemain→shot → GOAL/MISS</span>`;
    dashboard.appendChild(ghidraBanner);
    ...
    infoBanner.innerHTML = "⚠️ TABEL DI BAWAH = <strong>SAMPLE 1x (TIDAK STABIL)</strong> — contoh tunggal. <strong style='color:#0f0;'>HASIL STABIL 200x ADA DI KOTAK BIRU DI BAWAH</strong> — skor paling sering + rata-rata 200x + winrate konsisten (WE10 Ghidra Xorshift sadis ★5)";
        ...
        try {
          ...
          bulkWrap.innerHTML = `
            <div class="pred-section-title" style="color:#ff0;border-color:#ff0;">🔁 BULK PREDICT — 100 / 200 / 1000x ITERASI (FREKUENSI SEPERTI GAME ASLI) — ANTI HANG</div>
             <div style="font-size:0.6rem;color:#888;margin-bottom:6px;">Jalankan prediksi berkali-kali dengan Xorshift32 — lihat pemain apa yang paling sering muncul. <strong>Chunked yield tiap 16 task → tidak hang meski 200x×8 match.</strong></div>
            <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-bottom:8px;">
              <label style="font-size:0.65rem;color:#ccc;">Iterasi:</label>
              <input id="bulkIterations" type="number" value="200" min="10" max="50000" step="10" style="width:90px;background:#000;color:#0f0;border:1px solid #555;padding:4px;font-family:var(--font-mono);" />
              ...
              <span style="font-size:0.6rem;color:#aaa;">Skor WE10 pure sim (bukan dummy) — 10 s.d. 50000, input 10000 = 10000x (Xorshift Ghidra)</span>
            </div>
          ...
          setTimeout(()=>{
            ...
            if (btn && inp && out) {
              btn.addEventListener("click", async ()=>{
                const n = Math.max(10, Math.min(50000, parseInt(inp.value,10)||200));
                ...
                try {
                  ...
                  if(btnCancel) btnCancel.onclick = ()=>{ currentRunner.cancel(); if(progEl) progEl.innerHTML = `<span style="color:#f55;">⛔ Dibatalkan</span>`; };
                   const bulkSource = resolveActiveDataSource();
                   const validRows = validPreds.map(p=>({ row:p.row, homeCode:p.homeCode, awayCode:p.awayCode, homeName:p.homeName, awayName:p.awayName, homeFlag:p.homeFlag, awayFlag:p.awayFlag, exclude: StateManager.activeMemoryId !== null ? { memoryId: StateManager.activeMemoryId, gameNumber: bulkSource.gameNumber } : null }));
                  ...
                  if(res.cancelled){ out.innerHTML = `<div style="color:#f55;">⛔ Dibatalkan di ${res.completed}/${res.total}</div>`; }
                      ...
                      out.innerHTML = `
                         <div style="margin-bottom:8px;"><strong style="color:#ff0;">Hasil Bulk ${n}x sampling — async chunked, tidak hang:</strong> <span style="font-size:0.6rem;color:#888;">Skor dari event chance→pemain→shot, variasi antar iterasi via Xorshift32 seed unik</span></div>
      ...
      if (p.error || !p.prediction) {
        ...
        card.innerHTML = `
          ...
          ${p.p1Warning ? `<div style="background:#332200;border:1px solid #ffaa00;color:#ffcc66;padding:6px;font-size:0.7rem;margin-top:6px;">${Security.escapeHtml(p.p1Warning)}</div>` : ""}
           <div style="font-size:0.6rem;color:#888;margin-top:6px;">Daftar 57 valid: ${OFFICIAL_57_LIST.join(", ")}.</div>
        `;
      ...
      if(pred.stability){
        ...
        stabilityHtml = `
          ...
          <div style="background:${levelBg};border:2px solid ${levelColor};padding:8px;">
            ...
            ${warnLow}
            ${reasonHtml}
             <div style="font-size:0.55rem;color:#555;margin-top:4px;">Stability ≠ Confidence. Stability = konsistensi simulasi (Xorshift32 deterministik), reuse distribution pipeline. HIGH: top1≥~18% & top3≥~45% typical. Config: HIGH≥${PREDICTOR_CONFIG?.STABILITY?.HIGH||65}, MEDIUM≥${PREDICTOR_CONFIG?.STABILITY?.MEDIUM||40}</div>
          </div>`;
      }
      ...
      card.innerHTML = `
        ...
        <div style="background:#001a1a;border:1px solid #0ff;padding:8px;font-size:0.65rem;line-height:1.4;">
          ...
          <div style="margin-top:6px;background:#000;padding:6px;border:1px solid #333;font-family:var(--font-mono);font-size:0.6rem;">
            <strong style="color:#ff0;">AUDIT ROM SLPM_663.74 (jujur, 2026-08-30):</strong><br>
             RNG = Xorshift32 deterministik (implementasi model), bukan klaim decode penuh ROM<br>
             Team Model = teamRatings.js (estimasi 57 tim) + form tim • Player Model = playerAttributes.js (derived/estimated) • probs/markets/xG/scorelines/top-scorer berasal dari satu jalur event yang sama
          </div>
         ...
         <div class="pred-section-title">⚽ TOP GOALS / PREDIKSI PENCETAK GOL — PLAYER EVENT MONTE-CARLO (MC ${PREDICTOR_CONFIG?.PROBS_SIMS || 400})</div>
         <div style="font-size:0.6rem;color:#888;margin-bottom:4px;">Simulasi event-level • Prob = anytime • xG share = kontribusi gol • nama hanya dari roster project</div>
        ...
        <div class="pred-evidence-footer">
          ...
          <div><span style="color:#888;">Evidence:</span> Rating: ${pred.evidence.hasRating ? '✔ 57-valid' : '✘'} | Hist: ${pred.evidence.homeMatches}H/${pred.evidence.awayMatches}A | H2H: ${pred.evidence.h2hMatches}m | Context: ${pred.evidence.hasSimilarContext ? '✔' : '✘'} | Global xG: ${pred.evidence.globalAttack}</div>
           <div style="font-size:0.6rem;color:#555;margin-top:2px;">Source: WE10FullRoster.js (nama/posisi) + teams.js (negara) + teamRatings.js (ability estimasi) + form history • Xorshift32 deterministik • probs/markets/xG dari MC event yang sama</div>
        </div>
      `;
      dashboard.appendChild(card);
    });
    ...
    if (topGoalsForm) {
      if (!topGoalsForm.dataset.uiInit) {
        ...
        for (let i = 0; i < 16; i++) {
          ...
          const autoFillCountryAndGoals = (playerName) => {
            const countryValue = (cIn.value || '').trim();
            const teamCode = Object.entries(teamsDB).find(([, info]) => info.name.toLowerCase() === countryValue.toLowerCase())?.[0] || null;
            const player = lookupPlayerExact(playerName, teamCode);
            if (!player) return;
          ...
          pIn.addEventListener("input", () => {
            ...
            if (split) {
              const splitCountry = (cIn.value || '').trim();
              const splitTeamCode = Object.entries(teamsDB).find(([, info]) => info.name.toLowerCase() === splitCountry.toLowerCase())?.[0] || null;
              const player = lookupPlayerExact(split.name, splitTeamCode);
              if (player) {
            ...
            if (val.length >= 3) {
              const exactCountry = (cIn.value || '').trim();
              const exactTeamCode = Object.entries(teamsDB).find(([, info]) => info.name.toLowerCase() === exactCountry.toLowerCase())?.[0] || null;
              const exact = lookupPlayerExact(val, exactTeamCode);
              if (exact) autoFillCountryAndGoals(val);
            }
          });
          ...
          setupPlayerAutocomplete(pIn, () => {
            ...
            autoFillCountryAndGoals(formatted);
          });
        }
