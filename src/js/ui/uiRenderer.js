...
export const UIRenderer = {
  ...
  renderPredictionDashboard(predictions, container) {
    ...
    ghidraBanner.innerHTML = `✅ <strong>VALIDASI GHIDRA — BUKAN DUMMY</strong> | RNG <code>Xorshift32 FUN_0014d320 @003591d8 (seed 1, 118 jal, tmp=s^(s<<17); s=tmp^(tmp>>15); *2.3283064e-10)</code> | Dispatcher <code>FUN_0018a510 (RNG128/RNG1x20/RNG4, entry 0x448, +0x20/+0x22/+0x03/+0x446, heap *(004FFCA0)+0x337C)</code> | Chances <code>4-15</code> | Shot <code>0.38 + finishing*0.0042 + quality*0.42 - oppDef*0.0050</code> | Difficulty ★5 <code>*1.22</code> → skor sadis 5-10 | Pool <code>B1-B8 only</code> • <span style="color:#0f0;">Skor = event chance→pemain→shot → GOAL/MISS (bukan Math.random dummy)</span>`;
