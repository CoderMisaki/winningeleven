    ...
    for (const key of Object.keys(this.db.memories)) {
      ...
      for (const game of memory.games) {
        ...
        game.topGoals = Array.from({ length: 16 }, (_, i) => {
          const g = oldGoals[i] || {};
          const country = cleanText(g.country || "");
          const player = cleanText(g.player || "");
          const goals = cleanText(g.goals || "");
          if (!country && !player && !goals) return { country: "", player: "", goals: "" };
          const code = normalizeCountry(country);
          const resolved = canonicalizeRosterPlayer(code, player);
          const goalCount = Number(goals);
          if (!resolved || !Number.isInteger(goalCount) || goalCount <= 0) return { country: "", player: "", goals: "" };
          return { country: teamsDB[code]?.name || "", player: resolved.name, goals: String(goalCount) };
        });
