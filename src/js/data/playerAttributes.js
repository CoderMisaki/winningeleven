...
export function canonicalizeRosterPlayer(code, rawName) {
  const teamCode = String(code || "").toUpperCase();
  const player = findRosterPlayer(teamCode, rawName);
  if (!player) return null;
  return { code: teamCode, name: player.name, pos: player.pos, player };
}

export function isRosterPlayer(code, rawName) {
  return !!canonicalizeRosterPlayer(code, rawName);
}
