 ...
 *
 * CATATAN RNG: Xorshift32 FUN_0014d320 (via hybridPredict/ScoringRng; LCGRng = alias
 * Xorshift. LCG 1664525 hanya tersisa di LEGACY baseline bawah sebagai pembanding,
 * BUKAN produksi. Konstanta LCG 0 hits di SLPM_663.74, 118 hits jal 0014d320.)
 */
...
export function runWalkForwardBacktest(memoryId = 1, opts = {}) {
  ...
  for (let gIdx = 1; gIdx < games.length; gIdx++) {
    ...
    for (const m of targetGame.matches) {
      ...
      const actualScorers = targetGame.matches.length === 1 ? actualScorersForGame(targetGame) : [];
