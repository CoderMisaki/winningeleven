import { getTeamPlayers, getPlayerDatabase, normalizePlayerName, playerOverall } from "../data/playerAttributes.js";
...
function scoringIndexLocal(player){
  if(!player) return 0;
  const role = PLAYER_SCORING_CONFIG.ROLE_WEIGHT[String(player.pos||"").toUpperCase()];
  if (role == null) return 0;
  const base = 0.45*player.finishing + 0.30*player.positioning + 0.15*player.technique + 0.10*player.shotPower;
...
/** Resolve nama BANDAR/YES jadi record pemain lengkap (tim + atribut + scoringIndex). */
function resolveBandarRecords(bandar){
  const db = getPlayerDatabase();
  const recs = [];
  for(const b of (bandar||[])){
    const raw = String(b||"").trim();
    if(!raw) continue;
    const exact = resolvePlayerExactName(raw);
    if (!exact) continue;
    const target = normPlayer(exact);
    let player = null, team = null;
    for(const [code, players] of Object.entries(db)){
      for(const p of players){ if(normPlayer(p.name)===target){ player = p; team = code; break; } }
      if(player) break;
    }
    if(!player){
      const cand = [];
      for(const [code, players] of Object.entries(db)){
        for(const p of players){
          const pn = normPlayer(p.name);
          if(pn.includes(target) || target.includes(pn)) cand.push({ p, code });
        }
      }
      if(cand.length===1){ player = cand[0].p; team = cand[0].code; }
      else {
        const pref = cand.filter(c=> normPlayer(c.p.name).startsWith(target));
        if(pref.length===1){ player = pref[0].p; team = pref[0].code; }
      }
    }
    ...
    recs.push({
      ...
      scoringIndex: scoringIndexLocal(player)
    });
  }
  return recs;
}
    ...
    for(const entry of (profile.players||[])){
      ...
      pool.push({
        ...
        teamCode: code,
         teamName: team.name,
         flag: team.flag || "",
         finishing: entry.player.finishing,
         positioning: entry.player.positioning,
         technique: entry.player.technique,
         shotPower: entry.player.shotPower,
...
export const DraftService = {
  ...
  addBan(list, playerName, matches=null){
    ...
    if(!exists) return { ok:false, msg:`Pemain "${raw}" tidak ditemukan di roster 57 tim` };
     const exact = resolvePlayerExactName(raw);
     if (!exact) return { ok:false, msg:`Pemain "${raw}" tidak dapat diresolve ke roster` };
     list.push(exact);
  ...
  /** Rekomendasi adaptif 1-3 pemain — kalau ada BANDAR/YES: ranking DUEL (counter) vs pemain YES */
  recommend({ matches, bans=[], bandar=[], yesExisting=[] }){
    const bannedNorm = bans.map(b=>normPlayer(b)).filter(Boolean);
    // Resolve BANDAR/YES jadi record asli (tim + atribut) — resolusi exact, bukan substring liar
    const bandarRecs = resolveBandarRecords(bandar);
    const bandarTeams = new Set(bandarRecs.map(r=>r.team).filter(Boolean));
    // Fallback: kalau resolve gagal total, pakai union tim substring lama
    if(bandar.length && bandarTeams.size===0){
    ...
    // THREAT = pemain di slot YES/BANDAR yang harus dikalahkan (paling kuat kalau >1)
    const threat = bandarRecs.filter(r=>r.resolved).sort((a,b)=> b.scoringIndex - a.scoringIndex)[0] || null;

    let warning = "";
    let counterScoreFn = null;
    if(threat){
      // Skor duel: seberapa jauh kandidat MENGGULANGI threat, bukan ranking generik
      counterScoreFn = (p)=>(
        2.0*(p.scoringIndex - threat.scoringIndex) +
        1.2*(p.finishing - threat.finishing) +
        0.6*(p.positioning - threat.positioning) +
        0.4*(p.technique - threat.technique) +
        0.3*(p.shotPower - threat.shotPower) +
        p.selectionProbability*15
      );
      // Kandidat wajib layak duel: finishing & scoringIndex ≥ threat (fallback: mendekati)
      let contenders = pool.filter(p=> p.scoringIndex >= threat.scoringIndex && p.finishing >= threat.finishing);
      if(contenders.length===0){
        contenders = pool.filter(p=> p.scoringIndex >= threat.scoringIndex - 4);
        if(contenders.length===0){
          contenders = pool;
          warning = `Tidak ada pemain B1-B8 yang benar-benar unggul dari ${threat.name} — menampilkan lawan terbaik yang tersisa.`;
        }
      }
      pool = contenders.sort((a,b)=> counterScoreFn(b) - counterScoreFn(a));
    } else {
      pool = rankPool(pool);
      if(bandar.length) warning = `BANDAR "${bandar.join(", ")}" tidak ter-resolve ke roster — ranking generik dipakai (bukan duel).`;
    }
    ...
    const recommended = pool.slice(0, count).map((p,i)=>{
      const why = [];
      if(threat){
        const dFin = p.finishing - threat.finishing;
        const dIdx = p.scoringIndex - threat.scoringIndex;
        const dPos = p.positioning - threat.positioning;
        why.push(`DUEL vs YES/BANDAR ${threat.name} (${threat.team}): finishing ${p.finishing} vs ${threat.finishing} (${dFin>=0?"+":""}${dFin}), index ${p.scoringIndex} vs ${threat.scoringIndex} (${dIdx>=0?"+":""}${dIdx}), positioning ${p.positioning} vs ${threat.positioning} (${dPos>=0?"+":""}${dPos})`);
        why.push(dIdx>=0 && dFin>=0 ? `Unggul duel → layak melawan ${threat.name} di slot YES` : `Lawan terbaik yang tersisa untuk ${threat.name} (tidak ada yang unggul penuh)`);
        why.push(`Posisi ${p.pos} (role ${p.roleWeight.toFixed(2)}) — beda negara ${threat.team} → ${p.teamName} (${p.teamCode}) aman duel`);
      } else {
      ...
      if(bandar.length && threat) why.push(`Beda negara bandar [${bandar.join(", ")}] → blok ${[...bandarTeams].join(",")} lolos`);
      why.push(`Hanya dari tim di B1-B8 (${p.teamCode} ada di Schedule)`);
      const proof = counterScoreFn && threat
        ? `Counter ${counterScoreFn(p).toFixed(1)} = 2*Δidx(${p.scoringIndex}-${threat.scoringIndex}) + 1.2*Δfin(${p.finishing}-${threat.finishing}) + 0.6*Δpos(${p.positioning}-${threat.positioning}) + 0.4*Δtech + 0.3*Δpower + pick${(p.selectionProbability*100).toFixed(1)}%*0.15`
        : `Composite ${(p.selectionProbability*100 + p.scoringIndex*0.35).toFixed(1)} = pick${(p.selectionProbability*100).toFixed(1)}% + idx${p.scoringIndex}*0.35 + finishing bonus`;
      return { ...p, rank:i+1, why, proof };
    });
    return { ok:true, poolSize: pool.length, recommended, poolPreview: pool.slice(0,10), threat, warning };
  }
};
