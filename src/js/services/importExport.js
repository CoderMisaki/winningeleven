...
import { canonicalizeRosterPlayer } from "../data/playerAttributes.js";
...
export const ImportExportService = {
  ...
  m.score = normalizedScore;
          ...
          game.topGoals.forEach(g => {
            const countryRaw = typeof g.country === "string" ? g.country.trim() : "";
            const playerRaw = typeof g.player === "string" ? g.player.trim() : "";
            const goalsRaw = typeof g.goals === "string" ? g.goals.trim() : String(g.goals ?? "").trim();
            const hasAny = countryRaw || playerRaw || goalsRaw !== "";
            ...
            if (!countryRaw || !playerRaw || goalsRaw === "") {
              throw new Error(`Top goal pada Game ${game.gameNumber} harus berisi negara, pemain, dan gol.`);
            }

            const normCountry = normalizeCountry(countryRaw);
            if (!teamsDB[normCountry]) {
              throw new Error(`Negara pencetak gol tidak dikenal pada Game ${game.gameNumber}: ${countryRaw}`);
            }
            const rosterPlayer = canonicalizeRosterPlayer(normCountry, playerRaw);
            if (!rosterPlayer) {
              throw new Error(`Pemain ${playerRaw} tidak ada di roster ${teamsDB[normCountry].name}.`);
            }
            const goalsNum = Number(goalsRaw);
            if (!Number.isInteger(goalsNum) || goalsNum <= 0) {
              throw new Error(`Goals harus bilangan bulat positif pada Game ${game.gameNumber}.`);
            }
            g.country = teamsDB[normCountry].name;
            g.player = rosterPlayer.name;
            g.goals = goalsRaw;
          });
        });
