...
// Menggunakan LCGRng (= Xorshift32 FUN_0014d320, seed @003591d8) yang sama dengan
// predictor agar hasil bracket identik antara web & overlay TikTok Live jika seed sama.
// (Nama kelas LCGRng dipertahankan untuk kompatibilitas; isinya Xorshift, BUKAN LCG 1664525.)
//
...
//   - pemenang ditentukan deterministik: skor dipakai kalau ada, kalau tidak Xorshift
...
export const BaganRngService = {
  ...
  generateBracketFromMatches(matchRows, tiktokSeedRaw, opts = {}) {
    ...
    return {
      ...
      proof: {
        lcg: "Xorshift32 FUN_0014d320: tmp=s^(s<<17); s=tmp^(tmp>>15); float=s*2.3283064e-10 (LCGRng = alias Xorshift, bukan LCG 1664525)",
        shuffle: "Fisher-Yates via LCGRng.range(n) [Xorshift]",
