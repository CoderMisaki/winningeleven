...
export const PREDICTOR_CONFIG = {
  ...
  ENGINE_SOURCE: [
    ...
    "RNG         : Ghidra-synced Xorshift32 FUN_0014d320 @0x0014d320 state @003591d8 (seed 1, 118 call-sites, verified 2026-08-23 hasil_analisis_rng.txt). Update: tmp=s^(s<<17); s=tmp^(tmp>>15); float scaling *2.3283064e-10 *bound. Sinkron dengan dispatcher Konami Cup FUN_0018a510 (RNG 128/1/4 per match, entry 0x448, score @+0x20/+0x22, flag @+0x03/+0x446). Sebelumnya placeholder LCG NR 1664525 — kini diganti Xorshift agar 100% sinkron Ghidra."
  ].join(" "),
  GHIDRA_PROOF: "MCP verify 2026-08-23/30 (SLPM_663.74): FUN_0014d320 @0x0014d320 Xorshift seed@003591d8 (x^=x<<17; x^=x>>15; *2.3283064e-10) 118 hits jal C83405C0; FUN_0014d470 @0014d470 srand 2-word; FUN_0018a510 dispatcher Konami Cup (RNG128/RNG1x20/RNG4, entry 0x448, HOME+0x20/AWAY+0x22/STATUS+0x03/FLAG+0x446) pointer *(004FFCA0)+0x337C=7 match 0x1360B; FUN_0016e8d8 ceiling-div helper; FUN_00216ef0 table lookup 0x3C2100; 003bd800 pointer table; 003bd400 dump 0-0x1F4 belum terpetakan penuh ke 57 tim; 01323404 pairing verified P2S+RUNTIME u16 LE 32B; TikTokHook 00400000 forced table bypass RNG. Bukti valid tidak dihapus.",
  MAX_XG: 7.5,
  MIN_XG: 0.15,
  POISSON_CAP: 10,
...
// Legacy LCG kept as alias for reproducibility tests
export class LCGRng {
...
export function hybridPredict(homeCode, awayCode, excludeMemoryId = null, excludeGameNumber = null, opts = {}) {
  ...
  const modelParts = ["Ratings","Form","Ghidra-Xorshift"];
  ...
  try {
    ...
    rngProof = {
      mode: opts.sample === true ? "PLAYER_EVENT_SIM_BULK" : "PLAYER_EVENT_SIM_V7.1-GHIDRA",
      seed: fixtureSeed,
      ...
      method: `Ghidra-synced Xorshift32 FUN_0014d320 (seed@003591d8, tmp=s^(s<<17); s=tmp^(tmp>>15), *2.3283064e-10): ${chosenSim.homeChances + chosenSim.awayChances} chance → pemilihan pemain (roulette ROLE_WEIGHT x involvement x stamina x form) → shotProbability (finishing/attack/positioning/technique/power/stamina + quality*0.35 - oppDef*0.0065 + form*0.18) vs oppDef → goal/miss. Dispatcher FUN_0018a510 (RNG128/RNG1x20/RNG4, entry 0x448, +0x20/+0x22/+0x03/+0x446) verified; heap pointer *(004FFCA0)+0x337C. probs/markets/xG dari MC ${sims} sim.`,
      note: "RNG = Ghidra Xorshift32 FUN_0014d320 @003591d8 (hasil_analisis_rng.txt 118 call-sites, 2026-08-23). Atribut pemain derived/estimated (playerAttributes.js), bukan decode ROM penuh — bukti Ghidra valid tetap disimpan."
    };
  } catch (engineErr) {
...
export function whatIfPredict(homeCodeRaw, awayCodeRaw, homeGoalsRaw, awayGoalsRaw, opts = {}) {
  ...
  return {
    ...
    whatIfMeta: {
      ...
      lcg: "Xorshift32 FUN_0014d320: tmp=s^(s<<17); s=tmp^(tmp>>15); float=s*2.3283064e-10 (LCGRng = alias Xorshift, sinkron ROM)",
      ...
      audit: "Audit 2026-09-19 + re-verify Ghidra 2026-09-22: RNG = Xorshift32 FUN_0014d320 (118 hits jal, 0 hits LCG); atribut pemain derived/estimated (bukan decode ROM). xG display = rata-rata MC model yang sama."
    }
  };
}
