...
import {
  getPlayerDatabase, findRosterPlayer, playerOverall
} from "../data/playerAttributes.js";
import {
  PLAYER_SCORING_CONFIG, PLAYER_SCORING_MODEL_VERSION,
  scoringHashSeed, simulateMatch, simulateMatchToScore,
  runMatchMonteCarlo, getTeamScoringProfile, playerEventKey
} from "./playerScoring.js";
import { getScoringDatasetFingerprint } from "./scoringDataset.js";
...
export const OFFICIAL_57_LIST = Object.freeze(ALLOWED_CODES.map((code) => teamsDB[code].name));
...
export const PREDICTOR_CONFIG = {
  ...
  ENGINE_SOURCE: [
    "TEAM MODEL  : teamRatings.js (estimasi 57 tim) + form tim dari histori user dengan Bayesian shrinkage.",
    "PLAYER MODEL: playerAttributes.js — atribut derived dari posisi, rating tim, dan variasi deterministik.",
    "MATCH MODEL : playerScoring.js — chance → kualitas → pemain dari roster → shot probability vs defense → GOAL/MISS.",
    "IDENTITY    : nama/posisi hanya dari we10FullRoster.js; negara hanya dari teams.js.",
    "RNG         : Xorshift32 deterministik untuk reproduksi; bukan klaim decode penuh ROM."
  ].join(" "),
  GHIDRA_PROOF: "Static audit memory12: SLPM_663.74 adalah ELF MIPS stripped tanpa symbol predict; 0_TEXT.AFS memuat blok WEPLDATA; 12 modul ELF di MODULE.AFS juga stripped. Xorshift32 di model adalah implementasi deterministik, bukan klaim decode fungsi asli.",
  MAX_XG: 7.5,
  MIN_XG: 0.15,
  POISSON_CAP: 10,
  ...
  AUTO_APPLY: true // FIX: user expects B1-B8 & Top Goals auto-fill after PREDICT (sebelumnya false → harus klik APPLY manual)
};
...
// ============================================================
// 4. KONAMI CUP ENGINE — Xorshift32 Ghidra-synced (FUN_0014d320 @003591d8)
//    LCGRng (= Xorshift; nama dipertahankan untuk kompatibilitas) + PLAYER_DB_57.
//    Riwayat: port awal dari thinkpad/konami_cup.js memakai LCG NR 1664525 sebagai
//    placeholder; kini diganti Xorshift 1:1 ROM (0 hits LCG di SLPM, 118 hits jal 0014d320).
// ============================================================

// Xorshift32 Ghidra-synced — mirror FUN_0014d320 (seed @003591d8, 0x11/0x0F, *2.3283064e-10)
// Xorshift32
export class LCGRng {
  constructor(seed) { this.state = (seed >>> 0) || 1; if (this.state===0) this.state=1; }
  next() { let a=this.state; let tmp=(a ^ (a<<17))>>>0; this.state=(tmp ^ (tmp>>>15))>>>0; if(this.state===0) this.state=1; return this.state; }
  nextFloat() { return this.next()*2.3283064e-10; }
  range(n) { if(n<=0) return 0; const v=this.nextFloat()*n; let r=v|0; if(r>=n) r=n-1; return r &0xffff; }
...
function getRatingPrior(code) {
  ...
  if (!g || !g.source?.includes("ghidra-rom")) {
    ...
    if (!r) throw new Error(`Rating tim tidak tersedia untuk ${code}`);
...
export function extractDataset(excludeMemoryId = null, excludeGameNumber = null, excludeFromGameNumber = null) {
  ...
  const cacheKey = `${excludeMemoryId}|${excludeGameNumber}|${excludeFromGameNumber}|${getScoringDatasetFingerprint()}`;
  ...
  for (const mem of Object.values(memories)) {
    ...
    for (const g of mem.games) {
      ...
      for (const tg of g.topGoals || []) {
        ...
        if (!roster) continue;
        const key = roster.name + "|" + code;
        const goals = parseInt(tg.goals, 10) || 0;
        if (goals <= 0) continue;
        const entry = map.get(key) || { goals: 0, appearances: 0, teamCode: code, player: roster.name };
        entry.goals += goals;
        entry.appearances += 1;
        map.set(key, entry);
      }
...
function buildScorerRows({ homeCode, awayCode, sim, mc, opts = {} }) {
  ...
  for (const code of teams) {
    ...
    for (const e of profile.players) profileByName.set(playerEventKey({ teamCode: profile.code, playerName: e.name, playerIndex: e.index }), { entry: e, profile });
  }

  const mcByKey = new Map();
  if (mc) for (const p of mc.players) mcByKey.set(playerEventKey(p), p);

  // kumpulkan: pemain yang mencetak gol di sampel + pemain yang punya ekspektasi gol
  const rows = new Map();
  const touched = new Set();
  for (const e of sim.events) touched.add(playerEventKey(e));
  for (const key of mcByKey.keys()) touched.add(key);

  for (const key of touched) {
    const separator = key.indexOf("|");
    const teamPart = key.slice(0, separator);
    const name = key.slice(separator + 1);
    const slotSeparator = teamPart.lastIndexOf(":");
    const teamCode = slotSeparator >= 0 ? teamPart.slice(0, slotSeparator) : teamPart;
    const playerIndex = slotSeparator >= 0 ? Number(teamPart.slice(slotSeparator + 1)) : null;
    if (teamCode !== homeCode && teamCode !== awayCode) continue;
    if (!teamsDB[teamCode]) continue;
    ...
    const simGoals = sim.events.filter((e) => e.scored && playerEventKey(e) === key).length;
    const simChances = sim.events.filter((e) => playerEventKey(e) === key);
    ...
    rows.set(key, {
      // === field utama (UI baru) ===
      name, playerIndex, pos: player.pos, position: player.pos,
      teamCode, teamName: teamsDB[teamCode].name, flag: teamsDB[teamCode].flag || "",
      goals: simGoals,
      matchGoals: simGoals,
...
function limitScorerRows(rows, requestedLimit) {
  const limit = Number.isInteger(requestedLimit) && requestedLimit > 0
    ? requestedLimit
    : PREDICTOR_CONFIG.TOP_SCORERS_LIMIT;
  const positive = rows.filter((row) => (row.matchGoals || 0) > 0);
  const candidates = rows.filter((row) => (row.matchGoals || 0) <= 0);
  return [...positive, ...candidates].slice(0, Math.max(limit, positive.length));
}
...
export function buildTopScorersFromSimulation({ homeCode, awayCode, sim, mc, opts = {} }) {
  ...
  return limitScorerRows(rows, opts.limit);
}
...
//    Fungsi ini DIHAPUS — hybridPredict & whatIfPredict memakai roster + event simulation.
...
export function hybridPredict(homeCode, awayCode, excludeMemoryId = null, excludeGameNumber = null, opts = {}) {
  ...
  const modelParts = ["Ratings","Form","Xorshift32"];
  ...
  try {
    ...
    rngProof = {
      mode: opts.sample === true ? "PLAYER_EVENT_SIM_BULK" : "PLAYER_EVENT_SIM_V7.1",
      seed: fixtureSeed,
      ...
      method: `Xorshift32 deterministik: ${chosenSim.homeChances + chosenSim.awayChances} chance → pemilihan pemain dari roster (role × atribut × stamina × form) → shotProbability vs defense → goal/miss. probs/markets/xG dari MC ${sims} sim.`,
      note: "Atribut pemain derived/estimated; nama dan negara tetap canonical dari project. Ini bukan klaim decode fungsi predict asli ROM."
    };
  } catch (engineErr) {
    const reason = engineErr?.message || String(engineErr);
    throw new Error(`Engine prediksi pemain gagal: ${reason}`);
  }
  ...
  let winner = "DRAW";
  if (distResult.probs.home > distResult.probs.away + 0.07) winner = teamsDB[homeCode].name;
  else if (distResult.probs.away > distResult.probs.home + 0.07) winner = teamsDB[awayCode].name;
  // winner selalu konsisten dengan chosenScore (anti-monoton & bulk) — bukan hanya jika opts.sample
  if (chosenScore.home > chosenScore.away) winner = teamsDB[homeCode].name;
  else if (chosenScore.away > chosenScore.home) winner = teamsDB[awayCode].name;
  else winner = "DRAW";
  ...
  const debug = opts.debug ? {
    ...
    scorerModel: "WE10 Full Roster (57 tim × 11 pemain) + event-level player scoring + Bayesian historical smoothing",
    evidence,
...
//     via Xorshift32 FUN_0014d320 (LCGRng = alias Xorshift; konstanta LCG 0 hits
//     di ROM, 118 hits jal 0014d320, lihat GHIDRA_PROOF).
...
export function whatIfPredict(homeCodeRaw, awayCodeRaw, homeGoalsRaw, awayGoalsRaw, opts = {}) {
  ...
  const homeInput = String(homeGoalsRaw ?? "").trim();
  const awayInput = String(awayGoalsRaw ?? "").trim();
  const homeGoals = homeInput === "" ? NaN : Number(homeInput);
  const awayGoals = awayInput === "" ? NaN : Number(awayInput);
  if (!Number.isInteger(homeGoals) || homeGoals < 0 || homeGoals > 20) throw new Error("Gol HOME harus bilangan bulat 0-20.");
  if (!Number.isInteger(awayGoals) || awayGoals < 0 || awayGoals > 20) throw new Error("Gol AWAY harus bilangan bulat 0-20.");
  ...
  const winner = homeGoals > awayGoals ? teamsDB[homeCode].name : awayGoals > homeGoals ? teamsDB[awayCode].name : "DRAW";
  return {
    homeCode, awayCode,
    homeName: teamsDB[homeCode].name, awayName: teamsDB[awayCode].name,
    ...
    whatIfMeta: {
      ...
      lcg: "Xorshift32 FUN_0014d320: tmp=s^(s<<17); s=tmp^(tmp>>15); float=s*2.3283064e-10",
      ...
      audit: "Xorshift32 deterministik; atribut pemain derived/estimated; identitas canonical dari project; xG dari MC event yang sama."
    }
  };
}
...
export const PredictionService = {
  ...
  bulkPredict(dataSource, iterations = 100) {
    try {
      ...
      const excludeMemoryId = StateManager.activeMemoryId;
      const excludeGameNumber = dataSource?.gameNumber ?? null;
      ...
      for (let iter=0; iter<iterations; iter++) {
        for (const pm of perMatch) {
          const seed = hashStringToSeed(`${pm.homeCode}|${pm.awayCode}|${iter}|bulk|${PREDICTOR_CONFIG.MODEL_VERSION}`);
           const pred = hybridPredict(pm.homeCode, pm.awayCode, excludeMemoryId, excludeGameNumber, { deterministic:true, seed, sample:true });
          ...
          for (const pl of pred.topScorers) {
            if ((pl.matchGoals||0) <=0) continue;
             const k = playerEventKey(pl);
            const ex = pm.scorerMap.get(k);
            if (ex) { ex.hits+=1; ex.totalGoals+=pl.matchGoals; }
            else pm.scorerMap.set(k,{ name:pl.name, playerIndex:pl.playerIndex, pos:pl.pos, teamCode:pl.teamCode, flag:pl.flag, teamName:pl.teamName, hits:1, totalGoals:pl.matchGoals, weight:pl.weight, proofMath: pl.proofMath, reason:pl.reason, pickProb: pl.pickProb, totalWeight: pl.totalWeight });
            const gex = globalScorerFreq.get(k);
            if (gex) { gex.hits+=1; gex.totalGoals+=pl.matchGoals; gex.proofMath = pl.proofMath; }
            else globalScorerFreq.set(k,{ name:pl.name, playerIndex:pl.playerIndex, pos:pl.pos, teamCode:pl.teamCode, flag:pl.flag, teamName:pl.teamName, hits:1, totalGoals:pl.matchGoals, weight:pl.weight, proofMath: pl.proofMath, reason:pl.reason, pickProb: pl.pickProb, totalWeight: pl.totalWeight });
          }
        }
      }
      ...
      const globalRank = [...globalScorerFreq.values()].sort((a,b)=> b.hits - a.hits || b.totalGoals - a.totalGoals).map(x=>{
        ...
        // proof: kenapa muncul berkali-kali? weight/total * (1-exp(-xG)) ≈ freq expected
         const expectedApprox = x.pickProb ? `pick ${x.pickProb}% per chance → ${freqPct}% kemunculan (${x.hits}x/${iterations}) dari event player-level` : `freq ${freqPct}%`;
         return { ...x, freqPct, avgGoals: Number((x.totalGoals/x.hits).toFixed(2)), proof: expectedApprox, rngNote: `Xorshift32 seed=hash(home|away|iter|bulk) iter 0..${iterations-1}` };
      });
      ...
      const bulkRngProof = {
         lcg: "Xorshift32 FUN_0014d320: tmp=s^(s<<17); s=tmp^(tmp>>15); nextFloat=s*2.3283064e-10",
         seedFormula: "hashStringToSeed(home|away|iter|bulk|MODEL_VERSION) — deterministik, reproducible, tiap iter unik",
         whyFrequent: "Pemain muncul dari event chance → pemain → shot; bobot posisi, atribut, stamina, dan form hanya memilih kandidat, bukan nama buatan.",
         auditNote: "Roster dan nama negara dibatasi pada teams.js serta we10FullRoster.js; error simulasi tidak diganti dengan 0:0."
      };
