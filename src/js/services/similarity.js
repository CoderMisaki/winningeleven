...
Object.entries(countryAliases).forEach(([code, aliases]) => {
  ...
  ALIAS_LOOKUP_MAP.set(normalizeCountryKey(code), canonicalCode);
  aliases.forEach(alias => ALIAS_LOOKUP_MAP.set(normalizeCountryKey(alias), canonicalCode));
});

export function normalizeCountry(countryInput) {
  if (!countryInput) return "";
  const query = String(countryInput).trim();
  const normalized = normalizeCountryKey(query);
  return ALIAS_LOOKUP_MAP.get(normalized) || normalized.replace(/\s+/g, "").toUpperCase();
}
