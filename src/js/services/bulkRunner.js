...
export function createBulkRunner() {
  ...
  async function runChunked(validRows, iterations, onProgress) {
    ...
    const bulkRngProof = {
      lcg: "Ghidra-synced Xorshift32 FUN_0014d320 @003591d8: tmp=s^(s<<17); s=tmp^(tmp>>15); nextFloat=s*2.3283064e-10; range/bounded = float*bound clamp<bound (118 call-sites verified). Legacy NR-LCG dipertahankan sebagai reference.",
