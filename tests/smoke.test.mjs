...
// === APPLY PREDICTION (SPEC R — auto-fill true setelah fix) ===
let applyBarFound = !!$("#btnApplyBoth");
check("apply bar (APPLY SCORES / TOP GOALS) tersedia (auto-fill mode: skip jika AUTO_APPLY=true)", () => {
  // FIX: PREDICTOR_CONFIG.AUTO_APPLY sekarang true → skor auto-fill tanpa bar, jadi bar optional
  const bar = $("#btnApplyScores") || $("#btnApplyBoth") || $("#btnApplyGoals");
  const filled = stateMod.StateManager.homeQuery.matches.filter(x => /^\d+:\d+$/.test(x.score || "")).length;
  if (!bar && filled >= 8) return "auto-filled " + filled + "/8 (AUTO_APPLY=true)";
...
if (applyBarFound) {
  ...
  // AUTO_APPLY=true: tunggu auto-fill sudah terjadi (renderMatchGrid + persist)
  await new Promise((r) => setTimeout(r, 400));
}
