...
> **Catatan model prediksi (v7.2):** prediksi skor/pencetak gol memakai model level-pemain (`playerScoring.js` + `playerAttributes.js`). Nama/posisi hanya dari `we10FullRoster.js` dan negara hanya dari `teams.js`. Atribut pemain masih **derived/estimated**; RNG Xorshift32 deterministik untuk reproduksi, bukan klaim decode penuh fungsi ROM. Rincian & status data: [`PLAYER_MODEL_README.md`](PLAYER_MODEL_README.md).
...
| **What If** | Simulasi manual `Home 2 : 1 Away` -> lihat Top Goals otomatis dari **match engine level-pemain** (`playerScoring.js`). RNG Xorshift32 deterministik; skor dan nama selalu berasal dari event yang sama |
